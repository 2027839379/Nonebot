...
__plugin_author__ = "真寻作者复制过来的"
__plugin_task__ = {"zwa": "早午晚安"}


Config.add_plugin_config(
    "_task", "DEFAULT_ZWA", True, help_="被动 早午晚安 进群默认开关状态", default_value=False, type=bool
)
...
async def _():
    img = image(IMAGE_PATH / "naxida" / "20.git")
    await broadcast_group("[[_task|zwa]]诶嘿，早早早" + img, log_cmd="被动早午晚安")
    logger.info("每日早安发送...")
...
async def _():
    img = image(IMAGE_PATH / "naxida" / "16.gif")
    await broadcast_group(
        f"[[_task|zwa]]{NICKNAME}好累鸭，想休息了" + img, log_cmd="被动早午晚安"
    )
    logger.info("每日午安发送...")
...
async def _():
    img = image(IMAGE_PATH / "naxida" / "10.gif")
    await broadcast_group(
        f"[[_task|zwa]]{NICKNAME}该休息了哦，别晚睡" + img, log_cmd="被动早午晚安"
    )
    logger.info("每日晚安发送...")
