...
__plugin_author__ = "未知"
__plugin_task__ = {"group_welcome": "进群欢迎", "refund_group_remind": "退群提醒"}
Config.add_plugin_config(
    "invite_manager", "message", f"{NICKNAME}被强制拉进群，已自动退出！", help_="强制拉群后进群回复的内容.."
)
Config.add_plugin_config(
    ...
    if event.user_id == int(bot.self_id):
        ...
        if _flmt.check(event.group_id):
            ...
            if msg or img:
                ...
                await group_increase_handle.send(
                    "[[_task|group_welcome]]不管贫穷与富有， 不管缺陷与完美， 不管疾病与健康， 都愿意终生入群吗？"
                    + image(
                        IMAGE_PATH
                        / "jinqun"
                        / random.choice(os.listdir(IMAGE_PATH / "jinqun"))
                    )
                )
...
async def _(bot: Bot, event: GroupDecreaseNoticeEvent):
    ...
    if event.sub_type == "kick":
        ...
        rst = f"{user_name} 被 {operator_name} 送走了，啧……."
    try:
