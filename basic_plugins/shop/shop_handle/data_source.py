...
async def create_shop_help() -> str:
    ...
    for goods in goods_lst:
        if goods.goods_limit_time == 0 or time.time() < goods.goods_limit_time:
            _list.append(goods)
    # A = BuildImage(1100, h, color="#ffffff")
    total_n = 0
    image_list = []
    for idx, goods in enumerate(_list):
        name_image = BuildImage(
            580, 40, font_size=25, color="#eda1ed", font="CJGaoDeGuo.otf"
        )
        await name_image.atext(
            (15, 0), f"{idx + 1}.{goods.goods_name}", center_type="by_height"
        )
        await name_image.aline((380, -5, 280, 45), "#87CEFA", 5)
        ...
        if goods.goods_discount != 1:
            ...
            await name_image.atext((440, 15), str(discount_price), (65,105,225))#金币颜色
        else:
            await name_image.atext(
                (440, 0),
        ...
        des_image = None
        font_img = BuildImage(
            600, 80, font_size=20, color="#87CEFA", font="CJGaoDeGuo.otf"
        )
        ...
        if goods.goods_description:
            ...
            if desc[-1] == "\n":
                desc = desc[:-1]
            des_image = await text2image(desc, color="#87CEFA")
        ...
        if goods.goods_limit_time > 0:
            ...
            _limit_time_logo = BuildImage(
                40, 40, background=f"{IMAGE_PATH}/tuku_10/no.png"#限时
            )
            await bk.apaste(_limit_time_logo, (_w + 50, 0), True)
            await bk.apaste(
                BuildImage(0, 0, plain_text="限时购买", font_size=23, font="CJGaoDeGuo.otf", font_color=(220,20,60),),
                (_w + 90, 10),
                True,
            )
        ...
        if goods.goods_discount != 1:
            ...
            _discount_logo = BuildImage(
                30, 30, background=f"{IMAGE_PATH}/tuku_10/开.png"
            )
            await bk.apaste(_discount_logo, (_w + 50, 10), True)
            await bk.apaste(
                BuildImage(0, 0, plain_text="限时折扣", font_size=23, font="CJGaoDeGuo.otf", font_color=(85,156,75),),
                (_w + 90, 15),
                True,
            )
            await bk.apaste(
        ...
        if goods.daily_limit != 0:
            ...
            _daily_limit_logo = BuildImage(
                35, 35, background=f"{IMAGE_PATH}/tuku_10/限购.png"
            )
            await bk.apaste(_daily_limit_logo, (_w + 50, 10), True)
            await bk.apaste(
                BuildImage(0, 0, plain_text="每日限购", font_size=23, font="CJGaoDeGuo.otf", font_color=(255, 20, 147),),
                (_w + 90, 20),
                True,
            )
            await bk.apaste(
        ...
        if total_n < n:
            total_n = n
        if n:
            await bk.aline((650, -1, 650 + n, -1), "#87CEFA", 5)
            # await bk.aline((650, 80, 650 + n, 80), "#87CEFA", 5)
    ...
    for img in image_list:
        h += img.h + 10
    A = BuildImage(1100, h, color="#87CEFA")
    for img in image_list:
    ...
    shop_logo = BuildImage(250, 100, background=f"{IMAGE_PATH}/tuku_10/商店主题.png")#主题
    shop = BuildImage(w, h, font_size=20, color="#87CEFA")
    zx_img = BuildImage(0, 0, background=f"{IMAGE_PATH}/tuku_10/bizhi1.png")
    ...
    shop.text(
        ...
        "注【通过 序号 或者 商品名称 购买】", (255,0,255)
    )
    shop.text((20 + zx_img.w, h - 100), "神秘药水\t\t售价：10000000金币\n\t\t据说可以对喜欢的人进行涩涩~")
    return shop.pic2bs4()
