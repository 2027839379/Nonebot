...
async def create_help_image():
    ...
    task_image = await text2image(task_str, padding=5, color=(144, 238, 144))
    task_image = await help_template("被动任务", task_image)
    image_list.append(task_image)
    image_group, _ = group_image(image_list)
    A = await build_sort_image(image_group, color="#c0ffa2", padding_top=180)
    await A.apaste(
        BuildImage(0, 0, font="CJGaoDeGuo.otf", plain_text="超级用户帮助", font_color=(255,105,180), font_size=50),
        (50, 30),
        True,
    )
    await A.apaste(
