...
async def check_update(bot: Bot) -> Tuple[int, str]:
    ...
    if data:
        ...
        if _version != latest_version:
            ...
            logger.info(f"检测{NICKNAME}已更新，当前版本：{_version}，最新版本：{latest_version}")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]),
                message=f"检测{NICKNAME}已更新，当前版本：{_version}，最新版本：{latest_version}\n" f"开始更新.....",
            )
            logger.info(f"开始下载{NICKNAME}最新版文件....")
            ...
            if await AsyncHttpx.download_file(tar_gz_url, zhenxun_latest_tar_gz):
                logger.info(f"下载{NICKNAME}最新版文件完成....")
                error = await asyncio.get_event_loop().run_in_executor(
                    None, _file_handle, latest_version
                )
                if error:
                    return 998, error
                logger.info(f"{NICKNAME}更新完毕，清理文件完成....")
                logger.info(f"开始获取{NICKNAME}更新日志.....")
                ...
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
                    message=Message(
                        f"{NICKNAME}更新完成，版本：{_version} -> {latest_version}\n"
                ...
                logger.warning(f"下载{NICKNAME}最新版本失败...版本号：{latest_version}")
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
                    message=f"下载{NICKNAME}最新版本失败...版本号：{latest_version}.",
                )
        else:
            logger.info(f"自动获取{NICKNAME}版本成功：{latest_version}，当前版本为最新版，无需更新...")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]),
                message=f"自动获取{NICKNAME}版本成功：{latest_version}，当前版本为最新版，无需更新...",
            )
    else:
        logger.warning(f"自动获取{NICKNAME}版本失败....")
        await bot.send_private_msg(
            user_id=int(list(bot.config.superusers)[0]), message=f"自动获取{NICKNAME}版本失败...."
        )
    return 999, ""
...
def _file_handle(latest_version: str) -> str:
    ...
    logger.info(f"开始解压{NICKNAME}文件压缩包....")
    tf = tarfile.open(zhenxun_latest_tar_gz)
    tf.extractall(temp_dir)
    logger.info(f"解压{NICKNAME}文件压缩包完成....")
...
async def get_latest_version_data() -> dict:
    for _ in range(3):
        try:
            ...
            logger.error(f"检查更新{NICKNAME}获取版本失败 {type(e)}：{e}")
    return {}
