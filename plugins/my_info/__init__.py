...
get_my_group_info = on_command("我的信息", aliases={"/我的信息"}, permission=GROUP, priority=1, block=True)
my_level = on_command("我的权限", aliases={"/我的权限"}, permission=GROUP, priority=5, block=True)
...
async def _(event: GroupMessageEvent):
    if (level := await LevelUser.get_user_level(event.user_id, event.group_id)) == -1:
        await my_level.finish("你没有任何权限，非要说的话就是0", at_sender=True)
    await my_level.finish(f"您的权限等级为：{level}", at_sender=True)
